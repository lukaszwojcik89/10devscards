---
description: Instrukcje implementacji widoków frontendu w oparciu o plany implementacji
globs: *.astro,*.tsx
alwaysApply: false
---

# Instrukcje implementacji widoków frontendu
...
<implementation_plan>
{{implementation-plan}} <- zamień na referencję do planu implementacji widoku (np. @dashboard-view-implementation-plan.md)
</implementation_plan>

Teraz przejrzyj zasady implementacji:

<implementation_rules>
{{frontend-rules}} <- zamień na referencję do reguł frontendowych (np. @shared.md, @frontend.md, @astro.md, @react.md, @ui-shadcn-helper.md)
</implementation_rules>

Przejrzyj zdefiniowane typy:

<types>
{{types}} <- zamień na referencję do definicji DTOs (np. @types.ts)
</types>
...
2. Integracja API:
   ...
   - Obsłuż odpowiedzi z API i odpowiednio aktualizuj stan komponentów.
...
7. Optymalizacja wydajności:
   - Wdrożenie wszelkich optymalizacji określonych w planie lub zasadach.
